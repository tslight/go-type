...
- **NEW**: Strips to first occurrence of book title (removes additional boilerplate)
- Updates `manifest.json` with new file sizes

**Customization**:

To adjust how boilerplate is stripped, edit the `strip_gutenberg_boilerplate()` function in `strip_boilerplate.py`:
...
1. **First time setup**:
   ```bash
   python3 download_books.py     # Downloads 100 books with headers intact
   python3 strip_boilerplate.py  # Removes all boilerplate
   make all                        # Build Go app
   ```

2. **Add more books later**:
   ```bash
   python3 download_books.py     # Fetches any new books not in manifest
   python3 strip_boilerplate.py  # Strips boilerplate from new files
   make all                        # Rebuild
   ```
...
3. **Adjust stripping rules**:
   ...
   # Edit strip_boilerplate.py as needed
   python3 strip_boilerplate.py            # Apply updated rules
   ```
